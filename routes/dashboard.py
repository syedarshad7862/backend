...
async def get_dashboard_data(user_db= Depends(get_authenticated_agent_db)):
    ...
    #missing gender
    missing_gender = total_profiles - ( total_female + total_male )
    print(user)
    print("jk;dj",db)
    return JSONResponse(status_code=status.HTTP_200_OK, content={'total_profiles': total_profiles,'total_male': total_male, 'total_female': total_female, "missing_gender": missing_gender})